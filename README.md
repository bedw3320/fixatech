...
### Google Workspace Integration (gog) ✅ Installed
CLI tool for Gmail, Calendar, Drive, Contacts, Sheets, and Docs operations:

**Google Sheets** (Primary use for migrations):
```bash
# Read product catalog from Sheets
gog sheets get <sheetId> "Products!A1:Z1000" --json

# Write data mapping
gog sheets update <sheetId> "Mapping!A1:B10" --values-json '[["Source","Target"],["id","shopify_id"]]'

# Append migration progress
gog sheets append <sheetId> "Progress!A:C" --values-json '[["2026-01-27","Products","1000"]]'
```

**Google Docs** (Requirements and specs):
```bash
# Read requirements document
gog docs cat <docId>

# Export to text
gog docs export <docId> --format txt --out /tmp/requirements.txt
```
...
**Perfect for**: Product catalogs in Sheets, requirements in Docs, progress tracking
...
| **Skills** | ✅ 8 Installed | validation-framework, code-generator, mcp-integration, agent-identifier, figma, frontend-design, ralph-loop, gog |
